...
import {
  getMyComplaintDetailAction,
  type UserComplaintDetailItem,
...
export default function UserComplaintDetailPage() {
  ...
  const { t } = useLanguage();
  const [complaint, setComplaint] = useState<UserComplaintDetailItem | null>(
    null,
  );
  ...
  useEffect(() => {
    async function loadComplaint() {
      ...
      const result = await getMyComplaintDetailAction(complaintId);
      ...
      >
        <div>
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            View status, media, and officer responses
          </Text>
        </div>
