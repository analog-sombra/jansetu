...
const LOGIN_REQUIRED_ERROR = "Please login again to continue.";
...
export default function MeetingSectionPage() {
  ...
  async function loadMeetings() {
    ...
    if (!result.ok && result.error === LOGIN_REQUIRED_ERROR) {
      router.push("/login");
      return;
    }
    if (!result.ok) {
  ...
  function approveMeeting() {
    ...
    void (async () => {
      ...
      const result = await approveAdminMeetingAction({
  ...
  async function rejectMeeting(meeting: MeetingRecord) {
    ...
    const result = await rejectAdminMeetingAction({
      ...
      render: (_, row) => {
        ...
        const actions = [
          <Button
            key="view"
            size="small"
            onClick={() => router.push(`/mla/meeting-section/${row.id}`)}
          >
            View
          </Button>,
        ];
        ...
        {selectedMeeting && (
          <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
            <Alert
              type="info"
              showIcon
              title={`Citizen: ${selectedMeeting.citizenName ?? "-"}`}
              description={
