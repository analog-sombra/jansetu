...
const OFFICERS_BY_CATEGORY: Record<
  string,
...
const CLUSTER_SEEDS = [
  ...
  },
  {
    ...
    description:
      "Garbage piles not cleared from lane corner for multiple days.",
  },
  {
    area: "Raghubir Nagar",
    category: "WATER",
...
function generateDemoComplaints() {
  ...
  CLUSTER_SEEDS.forEach((seed, clusterIndex) => {
    for (let i = 0; i < 8; i += 1) {
      const status =
        STATUS_ROTATION[(clusterIndex + i) % STATUS_ROTATION.length];
      ...
      complaints.push({
        ...
        lng,
        createdAt: new Date(
          Date.UTC(2026, 3, (i % 20) + 1, 8 + (i % 6), 15),
        ).toISOString(),
        assignments:
  ...
  SINGLE_SEEDS.forEach((seed, singleIndex) => {
    ...
    complaints.push({
      ...
      lng,
      createdAt: new Date(
        Date.UTC(2026, 2, 20 + (singleIndex % 9), 9, 30),
      ).toISOString(),
      assignments:
...
export function buildPriorityClusters(
  rows: DemoComplaint[],
): PriorityCluster[] {
